...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 1. Fetch Settings
        const { data: settingsData, error: settingsError } = await supabase.from('punwarka_settings').select('*');
        ...
        // 2. Fetch Events
        const { data: eventsData } = await supabase.from('punwarka_events').select('*').order('created_at', { ascending: false });
        if (eventsData) {
        ...
        // 3. Fetch Staff
        const { data: staffData } = await supabase.from('punwarka_staff').select('*').order('id', { ascending: true });
        if (staffData) {
        ...
        // 4. Fetch News
        const { data: newsData } = await supabase.from('punwarka_news').select('*').order('date', { ascending: false });
        if (newsData) {
        ...
        // 5. Fetch Results
        const { data: resultsData } = await supabase.from('punwarka_results').select('*').order('year', { ascending: true });
        ...
        // 6. Fetch Facilities
        const { data: facilitiesData } = await supabase.from('punwarka_facilities').select('*');
        if (facilitiesData) {
